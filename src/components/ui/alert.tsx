	...
	"relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5",
	{
		variants: {
			variant: {
				...
				warn: "bg-warn-bg text-warn border-warn/30 [&>svg]:text-warn",
				critical: "bg-critical-bg text-critical border-critical/30 [&>svg]:text-critical",
				good: "bg-good-bg text-good border-good/30 [&>svg]:text-good",
			},
		},
		defaultVariants: {
			variant: "default",
