	...
	try {
		...
		if (!res.ok && res.status !== 206) {
			return {
				...
				actualHeight: null,
				detectedFormat: null,
				oversizedByFileSize: false,
				oversizedByResolution: false,
				oversizedReasons: [],
			};
		}
		...
		const totalFromRange =
			contentRange ? parseInt(contentRange.split("/")[1], 10) : null;
		const contentLength = res.headers.get("content-length");
		const fileSizeBytes =
			totalFromRange ?? (contentLength ? parseInt(contentLength, 10) : null);
		...
		return {
			...
			detectedFormat,
			oversizedByFileSize:
				fileSizeBytes !== null && fileSizeBytes > options.maxFileSizeBytes,
...
export async function analyzeImages(
	...
	const oversizedImages = withUrls
		.map((img) => {
			const check = checkByUrl.get(img.resolvedUrl!);
			return (
					check && (check.oversizedByFileSize || check.oversizedByResolution)
				) ?
					{ ...check, src: img.src }
				:	null;
		})
	...
	return {
		...
		nonModernFormatImages: images.filter(
			(i) => !i.isModernExtension && i.pictureSourceTypes.length === 0,
		),
		duplicateImages,
