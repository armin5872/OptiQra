...
function mapCategory(lhr: any, categoryKey: string) {
  ...
  for (const ref of category.auditRefs || []) {
    ...
    if (!audit || audit.scoreDisplayMode === 'informative' || audit.scoreDisplayMode === 'notApplicable' || audit.scoreDisplayMode === 'manual') continue;

    const auditScore = audit.score;
    if (auditScore === null || auditScore === undefined) continue;

    if (auditScore < 0.9) {
      const weight = Math.round((1 - auditScore) * (ref.weight ? Math.min(ref.weight, 12) + 4 : 10));
      issues.push({
        id: ref.id,
        title: audit.title,
        detail: cleanDescription(audit.description),
        fix: audit.displayValue ? `Current: ${audit.displayValue}` : 'See details for the specific elements affected.',
        weight: Math.max(3, weight),
        severity: auditScore < 0.5 ? 'critical' : 'warn',
        resolved: false,
      });
    } else if (ref.weight > 0) {
      passed.push({ id: ref.id, title: audit.title, severity: 'good', resolved: true });
    }
  }
...
export async function runPageSpeed(targetUrl: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`PageSpeed Insights request failed (${res.status}): ${body.slice(0, 200)}`);
  }
